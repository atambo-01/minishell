...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/18 11:28:12 by atambo            #+#    #+#             */
/*   Updated: 2025/01/18 11:28:14 by atambo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_pipe(t_cmd *cmd)
{
	...
	if (pid1 == 0)
	{
		...
		close(fd[1]);
		ft_execute(cmd->pc, 1); // Execute the previous command
	}
	pid2 = fork();
	if (pid2 == -1)
	{
	...
	if (pid2 == 0)
	{
		...
		close(fd[0]);
		ft_execute(cmd->nc, 0); // Execute the next command
	}
